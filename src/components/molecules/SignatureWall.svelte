<script lang="ts">
        ...
        function initCanvas() {
                ...
                ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#ffffff';
                ctx.lineWidth = 3;
                ctx.lineCap = 'round';
                ctx.lineJoin = 'round';
        ...
        $: if (signatureGridEl) updateGridColumns();
</script>
...
<section id="signature-wall" class="wrapper signature-wall">
        ...
        <div class="signature-grid" aria-live="polite" bind:this={signatureGridEl}>
                {#each shuffledSignatures as signature, index (signature.id)}
                        <div class="signature-cell" style={`--rotation: ${getSignatureRotation(signature.id)}deg; --order: ${getSignatureOrder(signature.id)}; --stagger-x: ${getRowStagger(index)}px`}>
                                <Tooltip tip={getSignatureTip(signature)}>
                                        <button class="signature-item" type="button" aria-label={`Signature by ${signature.name}`}>
                                                <span class="signature-ink" style={`--signature-image: url('${signature.signature_data}'); --ink-opacity: ${getSignatureInkOpacity(signature.id)}`}></span>
                                        </button>
                                </Tooltip>
                        </div>
                {/each}
...
<style lang="scss">
        .signature-wall { margin-bottom: 2rem; }
        .signature-header {
                ...
                p { font-size: 0.95rem; }
                @media (max-width: 768px) { flex-direction: column; gap: 1rem; p { font-size: 0.82rem; } }
        }
        .notice { font-size: 0.9rem; margin-bottom: 0.75rem; color: var(--text-secondary); }
        .cta {
                font-family: var(--font-two); font-size: 1rem; padding: 0.7rem 1.1rem; border-radius: 12px; border: 1px solid var(--elevation-four); background: var(--elevation-one); color: var(--text-primary); cursor: pointer;
                &:hover { filter: brightness(108%); transform: translateY(-1px); }
        }
        .signature-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 0.35rem; min-height: 8.5rem; }
        .signature-cell { display: flex; justify-content: center; transform: translateX(var(--stagger-x)); }
        .signature-item { background: transparent; border: 0; cursor: pointer; transform: rotate(var(--rotation)); &:hover { transform: rotate(0deg); } }
        .signature-ink {
                display: block; width: clamp(92px, 15vw, 160px); height: 64px; background-color: var(--accent);
                mask-image: var(--signature-image); -webkit-mask-image: var(--signature-image); mask-size: contain; -webkit-mask-size: contain; mask-repeat: no-repeat; -webkit-mask-repeat: no-repeat; mask-position: center; -webkit-mask-position: center; opacity: var(--ink-opacity, 0.82);
        }
        ...
        .modal-content { background: var(--bg-color); border: 1px solid var(--elevation-four); border-radius: 16px; padding: 1.5rem; max-width: 460px; width: 90%; max-height: 90vh; overflow-y: auto; }
        .modal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; h3 { margin: 0; } }
        ...
        .modal-body { display: flex; flex-direction: column; gap: 0.5rem;
                label { font-size: 0.85rem; color: var(--text-secondary); margin-top: 0.5rem; }
                input { padding: 0.65rem; border-radius: 10px; border: 1px solid var(--elevation-four); background: var(--elevation-one); color: var(--text-primary); font-size: 0.9rem; }
        }
        .canvas-container { position: relative; border: 2px dashed var(--elevation-four); border-radius: 12px; background: var(--elevation-one); }
        .signature-canvas { width: 100%; height: 180px; cursor: crosshair; touch-action: none; display: block; }
        .clear-btn { position: absolute; bottom: 8px; right: 8px; background: var(--elevation-two); border: 1px solid var(--elevation-four); color: var(--text-secondary); font-size: 0.7rem; padding: 0.3rem 0.7rem; border-radius: 6px; cursor: pointer; }
        ...
        .submit-btn { background: var(--accent); border: none; color: white; padding: 0.6rem 1.2rem; border-radius: 10px; cursor: pointer; }
</style>