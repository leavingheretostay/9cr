...
{#if activeSheet >= 0}
        <div class="comments-overlay" on:click={closeSheet}><div class="comments-sheet" on:click|stopPropagation>
                <div class="sheet-header"><h3>Comments</h3><button class="action-btn" on:click={closeSheet}>✕</button></div>
                <div class="sheet-body">
                        {#if comments[activeSheet]?.length > 0}
                                {#each comments[activeSheet] as comment, i}<div class="comment-bubble" on:touchstart={() => { longPressTimer = setTimeout(() => { startDelete(activeSheet, i); }, 800); }} on:touchend={() => clearTimeout(longPressTimer)} on:touchmove={() => clearTimeout(longPressTimer)}><span class="comment-name">{comment.name}</span><p class="comment-text">{comment.text}</p></div>{/each}
